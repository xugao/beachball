...
export type PromptRepsonse = { comment: string; type: ChangeType };
...
export async function promptForChange(options: BeachballOptions) {
  ...
  const recentCommitMessages = getRecentCommitMessages(branch, cwd) || [];
  ...
  for (let pkg of changedPackages) {
    ...
    const { customChangeFilePrompt } = packageInfo.options;
    if (customChangeFilePrompt) {
      response = await customChangeFilePrompt({ disallowedChangeTypes, recentCommitMessages });
    } else {
      ...
      const descriptionPrompt: prompts.PromptObject<string> = {
        ...
        suggest: input => {
          return Promise.resolve([...recentCommitMessages.filter(msg => msg.startsWith(input)), input]);
        },
      };
